...
object ChatController {
  ...
  suspend fun apiSetUserContactReadReceipts(u: User, userReadReceiptSettings: UserReadReceiptSettings) {
    val r = sendCmd(u.remoteHostId, CC.ApiSetUserContactReadReceipts(u.userId, userReadReceiptSettings))
    if (r.result is CR.CmdOk) return
    throw Exception("failed to set read receipts for user contacts ${r.responseType} ${r.details}")
  }

  suspend fun apiSetUserGroupReadReceipts(u: User, userReadReceiptSettings: UserReadReceiptSettings) {
    val r = sendCmd(u.remoteHostId, CC.ApiSetUserGroupReadReceipts(u.userId, userReadReceiptSettings))
    if (r.result is CR.CmdOk) return
    throw Exception("failed to set read receipts for user groups ${r.responseType} ${r.details}")
  }
...
sealed class CC {
  ...
  class ApiSetUserContactReadReceipts(val userId: Long, val userReadReceiptSettings: UserReadReceiptSettings): CC()
  class ApiSetUserGroupReadReceipts(val userId: Long, val userReadReceiptSettings: UserReadReceiptSettings): CC()
...
data class ChatSettings(
  ...
  val sendReadRcpts: Boolean?,
  val favorite: Boolean
) {
  companion object {
    val defaults: ChatSettings = ChatSettings(enableNtfs = MsgFilter.All, sendRcpts = null, sendReadRcpts = null, favorite = false)
  }
}
...
@Serializable
data class UserReadReceiptSettings(val enableReadRcpts: Boolean, val clearReadRcptOverrides: Boolean)
