...
getDirectUnreadItemIds :: DB.Connection -> User -> ContactId -> IO [ChatItemId]
getDirectUnreadItemIds db User {userId} contactId =
  map fromOnly
    <$> DB.query
      db
      [sql|
...
updateDirectChatItem_ db userId contactId ChatItem {meta, content} msgId_ = do
  ...
  DB.execute
    ...
    |]
    ((content, itemText, itemStatus, BI itemDeleted', itemDeletedTs', BI itemEdited, BI <$> itemLive, updatedAt) :. ciTimedRowAt updatedAt itemTimed :. (userId, contactId, itemId))
...
updateGroupChatItem_ db User {userId} groupId ChatItem {content, meta} msgId_ = do
  ...
  DB.execute
    ...
    |]
    ((content, itemText, itemStatus, BI itemDeleted', itemDeletedTs', BI itemEdited, BI <$> itemLive, updatedAt) :. ciTimedRowAt updatedAt itemTimed :. (userId, groupId, itemId))
...
getChatItemTTL db User {userId} =
  -- Default to 1 day (86400 seconds) if user hasn't set a custom TTL
  fmap (fromMaybe 86400 . join) . maybeFirstRow fromOnly $
