...
createUserRecordAt db (AgentUserId auId) Profile {displayName, fullName, image, preferences = userPreferences} activeUser currentTs =
  checkConstraint SEDuplicateName . liftIO $ do
    ...
    pure $ toUser $ (userId, auId, contactId, profileId, BI activeUser, order, displayName, fullName, image, Nothing, userPreferences) :. (BI showNtfs, BI sendRcptsContacts, BI sendRcptsSmallGroups, BI False, Nothing, Nothing, Nothing, Nothing)
...
updateUserReadReceiptSettings :: DB.Connection -> User -> UserReadReceiptSettings -> IO ()
updateUserReadReceiptSettings db User {userId} UserReadReceiptSettings {enableContacts, clearOverrides} = do
  DB.execute db "UPDATE users SET send_read_rcpts_contacts = ? WHERE user_id = ?" (BI enableContacts, userId)
  when clearOverrides $ DB.execute_ db "UPDATE contacts SET send_read_rcpts = NULL"
...
$(J.deriveJSON defaultJSON ''UserMsgReceiptSettings)

$(J.deriveJSON defaultJSON ''UserReadReceiptSettings)
