...
processChatCommand' vr = \case
  ...
  APIChatRead chatRef@(ChatRef cType chatId) -> withUser $ \_ -> case cType of
    CTDirect -> do
      ...
      (allUnreadIds, timedItems) <- withFastStore' $ \db -> do
        ...
        timedItemsWithTTL <- getDirectUnreadTimedItems db user chatId
        updateDirectChatItemsRead db user chatId
        timedItemsWithDeleteAt <- setDirectChatItemsDeleteAt db user chatId timedItemsWithTTL ts
        pure (allUnread, timedItemsWithDeleteAt)
      ...
      when (not $ null allUnreadIds) $ do
        mPayload <- withFastStore $ \db -> do
          ...
          case L.nonEmpty allUnreadIds of
            ...
            Just neItemIds -> do
              sharedIds <- liftIO $ getDirectSharedMsgIdsByItemIds db user chatId neItemIds
  ...
  APIChatItemsRead chatRef@(ChatRef cType chatId) itemIds -> withUser $ \_ -> case cType of
    CTDirect -> do
      user <- withFastStore $ \db -> getUserByContactId db chatId
      timedItems <- withFastStore' $ \db -> do
        timedItemsWithTTL <- updateDirectChatItemsReadList db user chatId itemIds
        setDirectChatItemsDeleteAt db user chatId timedItemsWithTTL =<< getCurrentTime
      forM_ timedItems $ \(itemId, deleteAt) -> startProximateTimedItemThread user (chatRef, itemId) deleteAt
